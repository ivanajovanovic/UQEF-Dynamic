...
def metrics(surrogate, list_of_dates_of_interest, standard_parameter_samples_matrix, standard_states_matrix, final_predicted_streamflow, final_observed_streamflow):
    ...
    surrogate_outputs = []
    for params, states in zip(standard_parameter_samples_matrix, standard_states_matrix):
        # params_col = params.reshape(-1, 1)
        
        combined = np.concatenate([params, states]).reshape(-1, 1)
        pce_output = float(chaospy.call(surrogate, combined))
