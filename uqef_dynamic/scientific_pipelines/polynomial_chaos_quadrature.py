...
def construct_polynomial_chaos_expansion(mean_state_values_dict,transport_map, scaler):
    ...
    nodes_r, weights_r = chaospy.generate_quadrature(3, distribution_r, rule="gaussian")
    ...
    countBounds = 0
    countNaN = 0
    for i in range(nodes_r.shape[1]):
        try:
            node = nodes_r[:, i:i+1]  # shape (n_params, 1)
            # print("Original sample: ", sample)
            node_q = inverse(node)
            
            if np.any(np.isnan(node_q)):
            ...
            if not all_within_bounds:
                ...
                nodes_q_list.append(node_q)
                # filter samples_r for those samples that are invalid
                nodes_r_list.append(node)
        except Exception as e:
            print(f"Exception for sample {i}: {e}")
            print("Input sample_r:", node.flatten())
    nodes_q = np.hstack(nodes_q_list)
    ...
    for node_q, node_r in zip(nodes_q.T, nodes_r_list):
        y = evaluate(node_q.reshape(-1, 1))
        ...
        valid_nodes_q.append(node_q.reshape(-1, 1))
        valid_nodes_r.append(node_r)
        valid_weights.append(weights_r[i]) 
        i += 1
    ...
    valid_nodes_r = np.hstack(valid_nodes_r)   
    ...
    model_approx = chaospy.fit_quadrature(expansion, valid_nodes_r, valid_weights, evaluations)
