...
" > $baseSourcePath/hbv_mc_500000_random_oldman_2004_2005.cmd

    #execute batch file
    sbatch $baseSourcePath/hbv_mc_500000_random_oldman_2004_2005.cmd

}

model="hbvsask"
opt_add="--parallel_statistics --save_all_simulations --sampleFromStandardDist --compute_Sobol_m --compute_Sobol_t --sc_poly_normed --store_gpce_surrogate_in_stat_dict" # --instantly_save_results_for_each_time_step  --read_nodes_from_file --regression
nodes=4
...
start_uq_sim "DWP" "DYNAMIC" "FCFS" "$uq_method" $q_order $p_order $mc_numevaluations "$model" "$opt_add" "$mpi_method" "$nodes" "$tasks_per_node" "$mid_time" "$uc" "$sampling_rule" "$sc_poly_rule" "$sc_quadrature_rule" "$parameters_file"  "$regression_model_type"
