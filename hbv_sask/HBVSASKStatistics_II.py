...
class HBVSASKSamples(object):
    def __init__(self, rawSamples, qoi_columns="Value", time_column_name="TimeStamp", **kwargs):
        ...
        self.qoi_columns = qoi_columns + [self.time_column_name, "Index_run"]
        ...
        for index_run, value in enumerate(rawSamples, ):
            ...
            if isinstance(value, tuple):
                ...
                if "gradient_matrix_dict" in value:
            ...
            if df_result is not None:
                if extract_only_qoi_columns:
                    list_of_single_df.append(df_result[self.qoi_columns])
                else:
...
class HBVSASKStatistics(Statistics):
    def __init__(self, configurationObject, workingDir=None, *args, **kwargs):
        ...
        # print(f"[DEBUGGING HBV STAT INFO] - {self.configurationObject}")
    ...
    def _infer_qoi_column_names(self, **kwargs):
        ...
        if self.mode == "continuous":
            if self.qoi == "GoF":
                for idx, single_qoi_column in enumerate(self.list_qoi_column):
                    if self.list_read_measured_data[idx]:
                        for single_objective_function_name_qoi in self.list_objective_function_names_qoi:
                            ...
                            self.list_qoi_column_processed.append(new_column_name)
                ...
                pass
                # self.list_qoi_column_processed.append(self.list_qoi_column)
        ...
        wrong_computation_of_new_qoi_columns = self.additional_qoi_columns_besides_original_model_output and \
                              len(self.list_qoi_column_processed) == 0
        assert not wrong_computation_of_new_qoi_columns

        if self.additional_qoi_columns_besides_original_model_output and len(self.list_qoi_column_processed) != 0:
            if always_process_original_model_output:
                self.list_qoi_column = self.list_original_model_output_columns + self.list_qoi_column_processed
            else:
                self.list_qoi_column = self.list_qoi_column_processed

        # print(f"[HBV STAT INFO] Statistics class will process the following QoIs:\n {self.list_qoi_column}\n"
        #       f"whereas the columns representing the model output itself are:{self.list_original_model_output_columns}"
        #       f"and additional QoI columns are { self.list_qoi_column_processed}. "
    ...
    def prepare(self, rawSamples, **kwargs):
        ...
        self.samples = HBVSASKSamples(rawSamples, qoi_column=list_of_columns_to_filter_from_results,
