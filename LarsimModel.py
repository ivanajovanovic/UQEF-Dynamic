...
class LarsimModelSetUp():
    ...
    def run_unaltered_sim(self, createNewFolder = False, write_in_file = True):
        ...
        if write_in_file:
            larsimInputOutputUtilities.write_dataFrame_to_file(self.df_unaltered_ergebnis, osp.abspath(
                    osp.join(self.working_dir, "df_unaltered_ergebnis.csv")))
    ...
    def _compare_measurements_and_unalteredSim(self):
        ...
        #####################################
        # hourly
        goodnessofFit_tuple = larsimDataPostProcessing.calculateGoodnessofFit(self.df_measured,
                                                                              self.df_unaltered_ergebnis,
        ...
        # daily
        goodnessofFit_tuple_daily = larsimDataPostProcessing.calculateGoodnessofFit(self.df_measured,
                                                                                    self.df_unaltered_ergebnis,
                                                                                    station = self.station_of_Interest,
                                                                                    type_of_output_of_Interest_measured = self.type_of_output_of_Interest_measured,
                                                                                    type_of_output_of_Interest = self.type_of_output_of_Interest,
        ...
        with open(gof_file_path, 'w') as f:
            ...
            f.write(line + "\n")
            f.write(
                    '{:.4f} {:.4f} {:.4f} {:.4f} \n'.format(result_tuple["RMSE"], result_tuple["BIAS"],
                                                            result_tuple["NSE"],
                                                            result_tuple["LogNSE"]))
            # f.write('{:.4f} {:.4f} {:.4f} {:.4f} \n'.format(result_tuple[k] for k in result_tuple.keys()))
            f.write('{:.4f} {:.4f} {:.4f} {:.4f} \n'.format(result_tuple_daily["RMSE"], result_tuple_daily["BIAS"],
                                                            result_tuple_daily["NSE"], result_tuple_daily["LogNSE"]))
...
class LarsimModel(Model):
    ...
    def run(self, i_s, parameters):  # i_s - index chunk; parameters - parameters chunk
        ...
        print("LarsimModel INFO {}: parameter: {}".format(i_s, parameters))
        ...
        for ip in range(0, len(i_s)):  # for each peace of work
            ...
            #####################################
            gt_dataFrame = larsimInputOutputUtilities.read_dataFrame_from_file(
                    osp.abspath(osp.join(self.working_dir, "df_measured.csv")))

            goodnessofFit_tuple = larsimDataPostProcessing.calculateGoodnessofFit(measuredDF = gt_dataFrame,
                                                                                  predictedDF = result,
                                                                                  station = self.station_of_Interest,
                                                                                  type_of_output_of_Interest_measured = self.type_of_output_of_Interest_measured,
                                                                                  type_of_output_of_Interest = self.type_of_output_of_Interest,
                                                                                  dailyStatisict = False)
            goodnessofFit_DailyBasis_tuple = larsimDataPostProcessing.calculateGoodnessofFit(measuredDF = gt_dataFrame,
                                                                                             predictedDF = result,
                                                                                             station = self.station_of_Interest,
                                                                                             type_of_output_of_Interest_measured = self.type_of_output_of_Interest_measured,
                                                                                             type_of_output_of_Interest = self.type_of_output_of_Interest,
            ...
            for gof_name in goodnessofFit_tuple[self.station_of_Interest].keys():
                ...
                index_parameter_gof_array.append(
                        round(Decimal(goodnessofFit_tuple[self.station_of_Interest][gof_name]), 4))
    ...
    def _multiple_short_larsim_runs(self, timeframe, timestep, curr_working_dir, parameters = None, index_run = 0):
        ...
        for i in range(number_of_runs + 1):
            ...
            local_resultDF = self._single_larsim_run(timeframe = single_run_timeframe,
                                                     curr_working_dir = curr_working_dir, parameters = parameters,
                                                     index_run = index_run, sub_index_run = i)

            # TODO Handle this more elegantly
            if local_resultDF is None:
                raise ValueError(
                        "LarsimModel ERROR: Process {}: The following Ergebnis file was not found - {}".format(
                                index_run,
                                result_file_path))
