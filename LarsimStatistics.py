...
class LarsimSamples(object):
    ...
    def __init__(self, rawSamples, station = None, type_of_output = 'Abfluss Messung', dailyOutput = "False"):
        ...
        list_of_single_df = []
        for index_run, value in enumerate(
                rawSamples, ):  # Important that the results inside rawSamples (resulted paths) are in sorted order and correspond to the parameters order
        ...
        print("LARSIM STAT INFO: Number of Unique TimeStamps (Hourly): {}".format(
            len(self.df_simulation_result.TimeStamp.unique())))
        ...
        if strtobool(dailyOutput):
            ...
            print("LARSIM STAT INFO: Number of Unique TimeStamps (Daily): {}".format(
                len(self.df_simulation_result.TimeStamp.unique())))
...
class LarsimStatistics(Statistics):
    ...
    def calcStatisticsForMc(self, rawSamples, timesteps,
        ...
        for key, val in groups.items():
            ...
            if regression:
                ...
                self.Abfluss[key]["Var"] = np.sum((discharge_values - self.Abfluss[key]["E"]) ** 2, axis = 0,
    ...
    def calcStatisticsForSaltelli(self, rawSamples, timesteps,
        ...
        for key, val in groups.items():
            ...
            discharge_values = samples.df_simulation_result.iloc[
                val.values].Value.values  # numpy array - for sartelli it should be n(2+d)x1
            ...
            self.Abfluss[key]["E"] = np.sum(extended_standard_discharge_values, axis = 0, dtype = np.float64) / (
                        2 * numEvaluations)
            self.Abfluss[key]["E_numpy"] = np.mean(discharge_values, 0)  # TODO!!!
    ...
    def _compute_Sobol_t(self):
        is_Sobol_t_computed = "Sobol_t" in self.Abfluss[
            self.keyIter[0]]  # hasattr(self.Abfluss[self.keyIter[0], "Sobol_t")
        return is_Sobol_t_computed

    def _compute_Sobol_m(self):
        is_Sobol_m_computed = "Sobol_m" in self.Abfluss[
            self.keyIter[0]]  # hasattr(self.Abfluss[self.keyIter[0], "Sobol_m")
        return is_Sobol_m_computed

    def get_measured_discharge(self, timestepRange = None):
        self.df_measured = larsimDataPostProcessing.read_process_write_discharge(
            df = os.path.abspath(os.path.join(self.working_dir, "df_measured.csv")), \
    ...
    def get_unaltered_discharge(self, timestepRange = None):
        self.df_unalatered = larsimDataPostProcessing.read_process_write_discharge(
            df = os.path.abspath(os.path.join(self.working_dir, "df_unaltered_ergebnis.csv")), \
            timeframe = timestepRange, \
            type_of_output = self.configurationObject["Output"]["type_of_output"], \
            station = self.configurationObject["Output"]["station"], \
            dailyOutput = strtobool(self.configurationObject["Output"]["dailyOutput"]))
    ...
    def plotResults(self, timestep = -1, display = False,
        ...
        fileName = self.generateFileName(fileName = fileName, fileNameIdent = ".html", directory = directory,
                                         fileNameIdentIsFullName = fileNameIdentIsFullName)
    ...
    def _plotStatisticsDict_plotly(self, unalatered = False, measured = False, station = "MARI",
        ...
        # Add enough colous such that we don't get errors in lines 348 / 351
        colors = ['darkred', 'midnightblue', 'mediumseagreen', 'darkorange', 'lightsalmon', 'aliceblue', 'aqua',
                  'aquamarine', 'darkturquoise', 'royalblue', 'saddlebrown', 'salmon', 'sandybrown']
        ...
        if measured:
            ...
            fig.add_trace(
                go.Scatter(x = self.df_measured['TimeStamp'], y = self.df_measured['Value'], name = "Q (measured)",
        ...
        fig.add_trace(go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["P10"] for key in self.keyIter],
                                 name = '10th percentile', line_color = 'indianred', mode = 'lines'), row = 1, col = 1)
        fig.add_trace(go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["P90"] for key in self.keyIter],
                                 name = '90th percentile', line_color = 'yellow', mode = 'lines'), row = 1, col = 1)
        ...
        fig.add_trace(
            go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["E_numpy"] for key in self.keyIter], name = 'E[Q]',
                       line_color = 'green', mode = 'lines'), row = 1, col = 1)
        ...
        fig.add_trace(go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["StdDev_numpy"] for key in self.keyIter],
                                 name = 'std. dev', line_color = 'darkviolet'), row = 2, col = 1)

        if is_Sobol_m_computed:
            for i in range(len(labels)):
                fig.add_trace(go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["Sobol_m"][i] for key in self.keyIter],
                                         name = labels[i], legendgroup = labels[i], line_color = colors[i]), row = 3,
                              col = 1)
        if is_Sobol_t_computed:
            for i in range(len(labels)):
                fig.add_trace(go.Scatter(x = pdTimesteps, y = [self.Abfluss[key]["Sobol_t"][i] for key in self.keyIter],
                                         legendgroup = labels[i], showlegend = False, line_color = colors[i]), row = 4,
                              col = 1)
    ...
    def _plotStatisticsDict_plotter(self, unalatered = False, measured = False, station = "MARI",
        ...
        if unalatered:
            ...
            plotter.plot(self.df_unalatered['TimeStamp'], self.df_unalatered['Value'],
                         label = "Q (unaltered simulation)")
        if measured:
        ...
        plotter.fill_between(pdTimesteps, [self.Abfluss[key]["P10"] for key in self.keyIter],
                             [self.Abfluss[key]["P90"] for key in self.keyIter], facecolor = '#5dcec6')
        ...
        plotter.subplot(412)
        plotter.plot(pdTimesteps, [self.Abfluss[key]["StdDev"] for key in self.keyIter],
                     label = 'std. dev. of the simulations')
        plotter.xlabel('time', fontsize = 13)
...
def _first_order(A, AB, B):
    ...
    return np.mean(B * (AB - A), axis = 0, dtype = np.float64) / np.var(np.r_[A, B], axis = 0, ddof = 1,
                                                                        dtype = np.float64)
